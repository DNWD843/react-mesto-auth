  ...
  return (
    <div className="start-page-container page__start-page-container">
      <form onSubmit={ props.onSubmit } className={ `form start-page-container__form form_type_${props.name}` } name={ `${props.name}-form` } id={ `${props.name}-form` } >
        ...
        { props.children }
        <button type="submit" disabled={ props.isDisabled } className={ `button ${props.isDisabled ? 'button_type_submit-start' : 'button_type_submit-start'} form__submit-button form__submit-button_type_start` } name={ `${props.name}-button` } value={ props.submitButtonText }>{ props.isLoading ? props.preloaderText : props.submitButtonText }</button>
        <Route path="/sign-up">
